...
public class OrdersController {
	...
	public ResponseEntity<Result> placeOrder(@RequestBody PlaceOrder model) {
		...
		for (OrderItem orderItem : model.getItems()) {
			ResponseEntity<Result> decreaseResult = productsService.decreaseAvailableCount(orderItem.getProductId(), orderItem.getQuantity());
		...
		ResponseEntity<Result> pay = paysService.pay(paymentCredentials);
		...
		shipService.orderPlaced(placedOrder);

		// 11. Notify stock tracking service
