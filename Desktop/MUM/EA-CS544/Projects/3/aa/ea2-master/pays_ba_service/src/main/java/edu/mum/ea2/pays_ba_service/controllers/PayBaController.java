...
public class PayBaController {
	...
	public ResponseEntity<Result> pay(@RequestBody Payment payment, HttpServletRequest request) {
		...
		if (!payment.getType().equals("ba")) return ResponseEntity.ok().body(new Result(false, "Payment type should be Bank Account: ba"));

		if (payment.getBankAccountNumber().length() != 8) return ResponseEntity.ok().body(new Result(false, "Bank account number should be 8 chars length"));
